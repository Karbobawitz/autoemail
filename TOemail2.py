#This is TOautoemail2.py, a script written to parse the .csv files generated by autoemail.sh
#and return the values needed for mailsmp to send the emails. You will need to customize
#this document with the emails of your workstudy students. 
#This file is identical to TOemail.py except it searches for events tomorrow, not next week.
#See README.txt for full instructions or is all else fails contact the creator of this script: 
...
with open('/Users/alkarbo/autoemail/TOemail.sh', 'a+') as f: #open a file to write some custom bash code
    for i in range(len(allinfo)): #for each show
        f.write("\nmail -s 'You have a show Tomorrow! (On %s)' " % allinfo[i][1]) #begin the mail command and specifiy the subject of the email
        ...
        f.write("alkarbo@davidson.edu < /Users/alkarbo/autoemail/TOemail{0}_2.txt\n".format(i)) #specifiy the text of the email as a file 
        f.write("echo 'TO Emails Sent\n'")
        f.write("rm /Users/alkarbo/autoemail/TOemail{0}_2.txt".format(i)) #make the bash script delete the txt files
